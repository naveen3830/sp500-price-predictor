...
const getRsiSliderColor = (rsiValue: number): string => {
  if (rsiValue < 30) {
    return "bg-emerald-500 dark:bg-bullish"
  }
  if (rsiValue > 70) {
    return "bg-rose-500 dark:bg-bearish"
  }
  return "bg-blue-600 dark:bg-cyanAccent"
}

interface IndicatorSummaryCardsProps {
  summary: IndicatorsResponse["summary"]
}

const IndicatorSummaryCards: React.FC<IndicatorSummaryCardsProps> = ({ summary }) => {
  const rsiValue = summary.rsi?.value ?? 50
  const rsiPos = Math.max(0, Math.min(100, rsiValue))
  const sliderColor = getRsiSliderColor(rsiValue)
      ...
      <div className="p-5 rounded-2xl bg-white/90 dark:bg-[#0E1526]/80 border border-slate-200/80 dark:border-white/[0.08] backdrop-blur-xl shadow-cardLight dark:shadow-terminal flex flex-col justify-between space-y-3 transition-colors duration-200">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative h-2 w-full bg-slate-100 dark:bg-[#131D33] rounded-full overflow-hidden border border-slate-200/80 dark:border-white/[0.06]">
            <div
              className={`h-full rounded-full transition-all duration-300 ${sliderColor}`}
              style={{ width: `${rsiPos}%` }}
            />
          </div>
        </div>
...
const RsiChartView: React.FC<ChartViewProps> = ({ records, isDark }) => (
...
const MacdChartView: React.FC<ChartViewProps> = ({ records, isDark }) => (
...
const BollingerChartView: React.FC<ChartViewProps> = ({ records, isDark }) => (
  <div className="h-72 w-full">
...
export const TechnicalAnalysis: React.FC<TechnicalAnalysisProps> = ({ symbol }) => {
  ...
  return (
    <div className="space-y-5">
      <IndicatorSummaryCards summary={summary} />
      ...
      <div className="p-6 rounded-2xl bg-white/90 dark:bg-[#0E1526]/80 border border-slate-200/80 dark:border-white/[0.08] backdrop-blur-xl space-y-5 shadow-cardLight dark:shadow-terminal transition-colors duration-200">
        ...
        {activeIndicatorView === "rsi" && <RsiChartView records={records} isDark={isDark} />}
        {activeIndicatorView === "macd" && <MacdChartView records={records} isDark={isDark} />}
        {activeIndicatorView === "bollinger" && <BollingerChartView records={records} isDark={isDark} />}
      </div>
    </div>
  )
}
