...
const getStrokeColor = (isDark: boolean, isPositive: boolean): string => {
  if (isDark) {
...
const VolumeTooltip: React.FC<VolumeTooltipProps> = ({ active, payload }) => {
  if (!active || !payload?.length) return null
  const item = payload[0].payload
  return (
...
export const PriceChart: React.FC<PriceChartProps> = ({ symbol }) => {
  ...
  const strokeColor = getStrokeColor(isDark, isPositive)
