...
export const Sidebar: React.FC = React.memo(() => {
  ...
  const [optimisticSymbol, setOptimisticSymbol] = useState<string | null>(selectedSymbol)

  useEffect(() => {
    setOptimisticSymbol(selectedSymbol)
  }, [selectedSymbol])
  ...
  // Instant tactile feedback on click: update selection in 0ms, defer heavy workspace load
  ...
  return (
    <aside
      className={`border-r border-slate-200/80 dark:border-white/[0.06] bg-white dark:bg-[#090E1A] flex flex-col h-[calc(100vh-4rem)] sticky top-16 shrink-0 z-20 overflow-hidden transition-[width] duration-300 ease-in-out will-change-[width] ${isCollapsed ? "w-16" : "w-80"
        }`}
    >
      {/* Persistent Content Container with fixed width to prevent text wrapping or reflow during collapse animation */}
      <div className="relative w-80 h-full flex flex-col">
        {/* Collapsed Rail View (Absolute pinned to left w-16) */}
        <div
          className={`absolute left-0 top-0 w-16 h-full flex flex-col items-center py-4 space-y-4 transition-opacity duration-200 z-20 ${isCollapsed ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
            }`}
        >
          <button
            type="button"
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition cursor-pointer relative ${activeView === "all"
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition cursor-pointer relative ${activeView === "watchlist"
        ...
        {/* Expanded Directory View */}
        <div
          className={`w-80 h-full flex flex-col transition-opacity duration-200 ${isCollapsed ? "opacity-0 pointer-events-none" : "opacity-100 pointer-events-auto"
            }`}
        >
          ...
          <div className="p-3 border-b border-slate-200/80 dark:border-white/[0.06] bg-slate-50/70 dark:bg-[#0C1322]/60 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1.5 p-1 rounded-xl bg-slate-200/60 dark:bg-[#080C14] border border-slate-200/80 dark:border-white/[0.05]">
              <button
                ...
                className={`flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-semibold transition-colors duration-100 cursor-pointer ${activeView === "all"
              ...
              <button
                ...
                className={`flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-semibold transition-colors duration-100 cursor-pointer ${activeView === "watchlist"
                    ? "bg-white dark:bg-[#162238] text-slate-900 dark:text-white shadow-sm border border-slate-200/60 dark:border-white/10"
                    : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                  }`}
              >
            ...
            {filteredStocks.length === 0 ? (
              ...
              paginatedStocks.map((stock) => (
                <StockCard
                  key={stock.symbol}
                  stock={stock}
                  isSelected={optimisticSymbol === stock.symbol}
